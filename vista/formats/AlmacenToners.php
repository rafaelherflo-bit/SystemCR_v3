...
$pdf->Cell(0, 10, utf8_decode('ALMACÉN | STOCK DE TONERS'), 0, 1, 'C');
$pdf->Ln(2);
...
function imprimirHeaders($pdf, $esColor = false)
{
  ...
  $pdf->Cell(15, 7, 'PARTE', 1, 0, 'C', true);
  $pdf->Cell(20, 7, 'CODIGO', 1, 0, 'C', true);
  $pdf->Cell(15, 7, 'RENDI', 1, 0, 'C', true);
  $pdf->Cell(25, 7, 'PROV', 1, 0, 'C', true);

  if ($esColor) {
    $pdf->Cell(60, 7, 'COMPATIBILIDAD', 1, 0, 'C', true);
...
$pdf->Cell(0, 8, "TONERS MONOCROMATICOS", 0, 1, 'L');
imprimirHeaders($pdf, false);

$consultaMono = consultaAlmacenP("WHERE AlmP_cat_id = 1 AND AlmP_subcat_id = 0 AND AlmP_prov_id != 11", "ORDER BY AlmP_descripcion ASC");
for ($i = 0; $i < $consultaMono['numRows']; $i++) {
  $AlmP = $consultaMono['dataFetch'][$i];
  list($tonerCodigo, $noParteToner, $rendimientoToner, $compatibilidadToner) = explode(" | ", $AlmP['AlmP_descripcion']);

  $pdf->Cell(15, 5, $noParteToner, 1, 0, 'C');
  $pdf->Cell(20, 5, $tonerCodigo, 1, 0, 'C', false, SERVERURL . 'Almacen/Toners/Editar/' . encryption($AlmP['AlmP_id']));
  $pdf->Cell(15, 5, $rendimientoToner, 1, 0, 'C');
  ...
  $pdf->Cell(70, 5, substr(utf8_decode($compatibilidadToner), 0, 50), 1, 0, 'L');
...
$pdf->SetFillColor(0);
$pdf->Cell(195, 6, "KYOCERA", 1, 1, 'C', true);
$pdf->SetFont('Arial', '', 8);
$pdf->SetTextColor(0);
$pdf->SetFillColor(255);
$pdf->Ln(1);
...
// ==========================================
$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(0, 8, "TONERS MULTICOLOR", 0, 1, 'L');
imprimirHeaders($pdf, true);

$substr = "";
$firsRow = 0;
$consultaColor = consultaAlmacenP("WHERE AlmP_cat_id = 1 AND AlmP_subcat_id != 0 AND AlmP_prov_id != 11", "ORDER BY AlmP_descripcion ASC, AlmP_subcat_id ASC");
for ($i = 0; $i < $consultaColor['numRows']; $i++) {
  ...
  $current_base = substr($tonerCodigo, 0, -1);
  ...
  // Fila de datos
  $pdf->Cell(15, 5, $noParteToner, 1, 0, 'C', true);
  $pdf->Cell(20, 5, $tonerCodigo, 1, 0, 'C', true, SERVERURL . 'Almacen/Toners/Editar/' . encryption($AlmP['AlmP_id']));
  $pdf->Cell(15, 5, $rendimientoToner, 1, 0, 'C', true);
  ...
  $tipoConfig = $arrayColorL[$AlmP['AlmP_subcat_id']] ?? ["?", 255, 255, 255];
  $pdf->SetFillColor($tipoConfig[1], $tipoConfig[2], $tipoConfig[3]);
  $pdf->Cell(60, 5, substr(utf8_decode($compatibilidadToner), 0, 42), 1, 0, 'L', true);
